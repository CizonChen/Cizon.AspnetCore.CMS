...
{
    public abstract class CizonRepositoryBase<TEntity> : IRepository<TEntity> where TEntity : Entity
    {  
        //定义数据访问上下文对象
        protected readonly CizonDbContext _dbContext;

        public CizonRepositoryBase(CizonDbContext dbContext)
        {
            _dbContext = dbContext;
        }
        public CizonRepositoryBase()
        {
            if(_dbContext==null)
            _dbContext = new CizonDbContext();
        }
        /// <summary>
        /// 获取实体集合
        /// </summary>
        ...
        /// <returns></returns>
        protected static Expression<Func<TEntity, bool>> CreateEqualityExpressionForId(string id)
        {
